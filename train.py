...
wandb_project = 'owt'
wandb_run_name = 'gpt2_moe' # 'run' + str(time.time())
# data
dataset = 'openwebtext'
...
if init_from == 'scratch':
    ...
    print("Initializing a new MoE model from scratch")
    # determine the vocab size we'll use for from-scratch training
    if meta_vocab_size is None:
        print("defaulting to vocab_size of GPT-2 to 50304 (50257 rounded up for efficiency)")
    model_args['vocab_size'] = meta_vocab_size if meta_vocab_size is not None else 50304
    model = GPT2LMHeadModel.from_pretrained('gpt2')
    model, router_losses, gptconf = createGPT2MoE(model, num_experts=num_experts, copy_mlp=False,router_loss=True)
    ...
    model, router_losses, gptconf = createGPT2MoE(model, num_experts=num_experts, copy_mlp=True,router_loss=True)
...
def estimate_loss():
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            X, Y = get_batch(split)
            with ctx:
                output = model(input_ids=X, labels=X) # labels are shifter inside model as per doc.
                loss = output.loss
                loss = (loss + sum([x.z_value+x.aux_value for x in router_losses]))
            losses[k] = loss.item()
